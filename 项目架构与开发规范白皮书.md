...
│   │   │   │   ├── AdminCourseOrderController.java  # 课程订单管理
...
│   │   │   ├── CourseOrderController.java    # 课程订单（通用）
...
│   │   │   ├── CourseOrder.java          # 课程订单
│   │   │   ├── CourseOrderStatus.java    # 课程订单状态枚举
│   │   │   ├── Package.java              # 套餐产品
...
| `AdminCourseOrderController` | `/api/admin/course-orders` | 课程订单管理 |
...
| `CourseOrderService` | `getByUserId()`, `refund()` |
...
| **RPC 风格** | `POST /api/admin/course-orders/{id}/refund` | 执行退款操作 |
...
Booking {
    ...
    packageOrderId: String  // 关联 CourseOrder ID
}
```

#### 5.2.4 CourseOrder（课程订单/套餐订单）
...
CourseOrder {
    ...
    status: CourseOrderStatus // ACTIVE/EXPIRED/COMPLETED/REFUNDING
}
```
...
3. **CourseOrder 与 Booking 的关系**：预约时扣减 CourseOrder 的剩余课时
...
- 添加 restoreCourseOrderSessions 方法
...
| `course_order` | 课程订单表 | CourseOrder |
...
| `CourseOrderStatus` | `ACTIVE`, `EXPIRED`, `COMPLETED`, `REFUNDING` | 课程订单状态 |
